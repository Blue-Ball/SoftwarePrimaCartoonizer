...
namespace AForge.Imaging.Filters
{
    ...
    public class ResizeBilinear : FilterResize
    {
        ...
        protected override unsafe void ProcessFilterArgb(BitmapData sourceData, BitmapData destinationData)
        {
            // get source image size
            int width = sourceData.Width;
            int height = sourceData.Height;

            int pixelSize = (sourceData.PixelFormat == PixelFormat.Format8bppIndexed) ? 1 : 4;
            int srcStride = sourceData.Stride;
            int dstOffset = destinationData.Stride - pixelSize * newWidth;
            double xFactor = (double)width / newWidth;
            double yFactor = (double)height / newHeight;
